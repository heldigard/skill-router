...
ROUTES: list[Route] = [
    ...
    Route(
        patterns=('\\b(dead code|unused code|c[oó]digo muerto|sin usar|unused (func|import|var))\\b',
         ...
         '(review|scan|check))\\b'),
        hint=('Quality sensors (ship-time): run `codescan` (`dead`/`sec`/`secrets`/`arch`/`all`) — dead-code '
         '(vulture/knip), SAST (semgrep), leaked secrets (gitleaks), import-rule violations '
