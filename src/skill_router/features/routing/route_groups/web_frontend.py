...
WEB_FRONTEND_ROUTES: list[Route] = [
    Route(
        patterns=(
            "\\b(payload|payloadcms|payload cms|payload\\.config|collectionconfig)\\b",
            "\\b(collections?|fields?|hooks?|access control)\\b.*\\b(payload|cms)\\b",
        ),
        hint=(
            "Skill: load `payload` for Payload CMS 3.x configs, collections, fields, hooks, access control, "
    ...
    Route(
        patterns=(
            "\\b(jenkins|jenkinsfile|jcasc|job dsl)\\b",
            "\\b(declarative pipeline|scripted pipeline|shared librar(?:y|ies))\\b",
        ),
        hint=(
            "Skill: load `jenkins` for Jenkinsfile, Declarative/Scripted Pipeline, shared libraries, JCasC, "
    ...
    Route(
        patterns=(
            "\\b(ag[- ]?grid|data grid|angular grid|celdas?|grid community)\\b",
            "\\b(row group|tree data|master detail|excel export|cell renderer)\\b.*\\b(ag[- ]?grid|grid)\\b",
        ),
        hint=(
            "Skill: load `ag-grid-community-angular` for AG Grid Community (free) patterns in Angular — setup "
    ...
    Route(
        patterns=(
            "\\b(react|reactjs|\\bjsx\\b|\\btsx\\b|next\\.?js|nextjs|server "
    ...
    Route(
        patterns=(
            "\\b(angular|ngfor|ngif|@component|@injectable|rxjs|signals?\\b|angular cli|ng serve|ng "
    ...
    Route(
        patterns=(
            "\\b(vue|vuejs|vue\\.js|script setup|composition "
            "api|composables?\\b|pinia|nuxt|defineprops|defineemits)\\b",
        ),
        hint=(
            "Skill: load `vue` for Vue 3.5 (Composition API, <script setup>, ref/reactive/computed/watch, "
    ...
    Route(
        ...
        hint=(
            "Skill: load `svelte` for Svelte 5 runes ($state/$derived/$effect, onclick, $props) + SvelteKit "
    ...
    Route(
        ...
        hint=(
            "Skill: load `tailwind` for Tailwind v4 (Oxide, CSS-first @theme config, responsive/container "
    ...
    Route(
        patterns=(
            "\\b(good interface|responsive layout|adaptive design|component architecture|web vitals|core web "
    ...
    Route(
        patterns=(
            "\\b(figma|pixel[- ]?perfect|dise[nñ]o figma|sync design|visual discrepancy|compara con "
            "figma|pantallazo|screenshot comparison|design fidelity)\\b",
        ),
        hint=(
            "Skill: load `figma-design-sync` or load `design-implementation-reviewer` or load "
