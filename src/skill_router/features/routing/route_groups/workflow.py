...
WORKFLOW_ROUTES: list[Route] = [
    ...
    Route(
        patterns=('\\b(antigravity|antigravity cli|google antigravity|agy3?-pro|gemini cli|agent-first '
         'coding|agent skills)\\b',
         '\\b(antigravity skills|antigravity hooks|antigravity plugins|antigravity artifacts)\\b'),
        hint=('Skill: load `antigravity` for Google Antigravity/CLI, agent skills, artifacts, long-context '
    ...
    Route(
        patterns=('\\b(wsl|wsl2|wslg|windows subsystem for linux|\\.wslconfig|wsl\\.conf|wsl\\.exe|/mnt/c|/mnt/wslg)\\b',
         '\\b(systemd.*wsl|wsl.*systemd|wsl.*dns|wsl.*network|wsl.*docker|wsl.*gui|wsl.*playwright)\\b'),
        hint=('Skill: load `wsl` for WSL/WSL2/WSLg setup, `.wslconfig`, `wsl.conf`, systemd, networking, '
